...
/// envelope (or any nested shape) changes in a non-additive way.
///
/// Faz S1 turned [CoreAudioSource] into a sealed hierarchy and renamed the
/// per-item discriminator (`'remote'` → `'http'`). No envelope bump: there
/// is no released app reading these JSONs yet, so a clean break beats the
/// migration ceremony. Future shape changes — especially additive ones for
/// HLS (S2) / Live (S3) — also extend the discriminator switch without
/// bumping this version. Bump only when an existing field renames or moves
...
/// Immutable ordered collection of [CoreAudioSource]s with a
...
extension type const CorePlayerQueue._(
  ...
  CorePlayerQueue.single(CoreAudioSource source) : this._(([source], 0));

  /// Empty queue. Useful as an initial state placeholder.
  const CorePlayerQueue.empty() : this._((const <CoreAudioSource>[], 0));

  /// The backing source list. Treat as immutable.
  List<CoreAudioSource> get sources => _.$1;
  ...
  CoreAudioSource? get current => isEmpty ? null : sources[currentIndex];

  /// Indexed read into [sources]. Throws if [index] is out of range.
  CoreAudioSource operator [](int index) => sources[index];
  ...
  static CorePlayerQueue fromJson(Map<String, Object?> json) {
    ...
    final items = <CoreAudioSource>[
      for (final raw in rawItems)
        if (raw is Map)
          CoreAudioSource.fromJson(raw.cast<String, Object?>())
        else
